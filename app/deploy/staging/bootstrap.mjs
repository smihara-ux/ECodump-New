// Dedicated staging bootstrap. Includes anonymous baseline and Narita model seed from setup.mjs.
// Run only against the dedicated staging Compose database; never import local setup.mjs.
import { mkdir, readFile, writeFile, chmod } from 'node:fs/promises';
import { existsSync } from 'node:fs';
import path from 'node:path';
import { createHash, randomBytes, scryptSync } from 'node:crypto';
import pg from 'pg';
const root='/srv/ecodump/server';
const local='/state';
if(process.env.ECODUMP_STAGING_BOOTSTRAP!=='dedicated-staging' || process.env.ECODUMP_DB_HOST!=='db' || process.env.ECODUMP_DB_NAME!=='ecodump_direct_validation') throw new Error('Dedicated staging target required');
for(const name of ['ECODUMP_DB_ADMIN_PASSWORD','ECODUMP_DB_API_PASSWORD','ECODUMP_DB_AUTH_PASSWORD']) if(!/^[a-f0-9]{64}$/.test(process.env[name]||'')) throw new Error(`Invalid independent staging secret: ${name}`);
await mkdir(local,{recursive:true,mode:0o700}); await chmod(local,0o700);
const admin={host:'db',port:5432,user:'ecodump_owner',database:'ecodump_direct_validation',password:process.env.ECODUMP_DB_ADMIN_PASSWORD};
...
await writeFile(path.join(local,'config.json'),JSON.stringify({admin,api:{...admin,user:'ecodump_api',password:process.env.ECODUMP_DB_API_PASSWORD},auth:{...admin,user:'ecodump_auth',password:process.env.ECODUMP_DB_AUTH_PASSWORD},port:6102,ids,naritaIds},null,2),{mode:0o600});
for (const [role, password] of [['ecodump_api', process.env.ECODUMP_DB_API_PASSWORD], ['ecodump_auth', process.env.ECODUMP_DB_AUTH_PASSWORD]]) {
 const literal = (await db.query('SELECT quote_literal($1) AS value', [password])).rows[0].value;
 await db.query(`ALTER ROLE ${role} PASSWORD ${literal}`);
}
await db.end();
console.log('Staging bootstrap complete. Baseline and Narita seed installed; credentials kept in /state.');
